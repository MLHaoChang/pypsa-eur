...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "`unsaved`, so a chat edit can be destroyed with no prompt. The fix "
        "belongs at the single dispatch site (chat_service.py:2948, where "
        "`tier` is already in scope) and must sit on the correct side of the "
        "confirmation gate — marking before it would mark even when the user "
        "DECLINES. strict=True so this fails loudly the moment it is fixed, "
