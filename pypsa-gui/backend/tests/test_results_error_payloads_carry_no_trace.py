...
When the per-carrier roll-up raises, the handler degrades gracefully rather
than 500ing — deliberately, so one bad carrier does not blank the Results tab.
...
def _exploding_rollup(monkeypatch):
    ...
    what runs. `_compute_economics_summary` is imported inside the handler body,
    so the patch has to land on the module that defines it.
    """
    ...
    monkeypatch.setattr(compare_router, "_compute_economics_summary", _raise)
    return boom
