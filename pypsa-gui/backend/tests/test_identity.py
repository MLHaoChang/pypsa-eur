...
def test_save_with_mismatched_expect_returns_409(client, tmp_projects_dir, install_network, session_ctx):
...
def test_save_active_project_with_matching_expect_ok(
    client, tmp_projects_dir, install_network, project_storage_dir, session_ctx
):
    install_network(build_network(), name="A")
...
def test_first_save_claims_unbound_network(client, tmp_projects_dir, install_network, session_ctx):
    ...
    assert session_ctx(client).loaded_project == "New"
    assert client.get("/api/network/meta").json()["loaded_project"] == "New"


def test_rebind_moves_binding_to_new_name(client, tmp_projects_dir, install_network, session_ctx):
    install_network(build_network(), name="A")
