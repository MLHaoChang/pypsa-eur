...
def _rewrite(conn, transform) -> None:
...
def upgrade() -> None:
    ...
    _rewrite(conn, to_relative)

    # Tolerate an index that already exists: a database built by `create_all`
    # carries the model's `UniqueConstraint`, and this migration is run twice
    # by its own idempotence test.
