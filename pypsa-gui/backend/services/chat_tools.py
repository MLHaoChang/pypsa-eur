...
# How many islands `diagnose_network` reports, and how many isolated buses.
# Carried over from the inline implementation this tool replaced: a 400-bus
# shrapnel network serialises past `_truncate_result`'s budget and comes back
# as a preview string, and a diagnosis the agent cannot read is not a
# diagnosis. `counts` in the report below is NOT capped, so the agent is still
...
def diagnose_network(include_buses: bool = False) -> dict:
    ...
    The island list is capped for the same reason — see `_MAX_ISLANDS_REPORTED`.
    """
    ...
    isolated = report["isolated_buses"]
