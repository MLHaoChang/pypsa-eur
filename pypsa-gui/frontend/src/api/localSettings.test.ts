...
describe('keyFieldPlaceholder', () => {
  it('prompts for a key when none is stored', () => {
    ...
    expect(keyFieldPlaceholder({ key_set: false, key_hint: null, key_redactable: null, log_path: '/l' }))
      .toBe('sk-ant-…')
  })

  it('shows the hint when one is available', () => {
    expect(keyFieldPlaceholder({ key_set: true, key_hint: '7f3a', key_redactable: true, log_path: '/l' }))
  ...
  it('still reports a stored key when the hint was withheld', () => {
    ...
    expect(keyFieldPlaceholder({ key_set: true, key_hint: null, key_redactable: null, log_path: '/l' }))
      .toBe('Key set')
  })
})
