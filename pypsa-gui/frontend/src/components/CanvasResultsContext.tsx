...
interface TSPayload { index: string[]; columns: string[]; data: number[][]; periods?: number[] }
...
export function CanvasResultsProvider({ children }: { children: ReactNode }) {
  ...
  // resultSource is included in the queryKey so the canvas re-fetches when
  ...
  const value = useMemo<OverlayData>(() => {
    ...
    // Cap the snapshot index against the actual data length; some payloads may
    // be shorter than n.snapshots (e.g. partial saves), in which case we
    // silently render the last available row rather than blowing up. Pull
    // length from whichever TS payload arrived first — handles link-only
    // networks (no Lines or Generators) gracefully.
    const len = ts?.data.length ?? lts?.data.length ?? lints?.data.length ?? lkts?.data.length ?? 0
    if (len === 0) return EMPTY
    const idx = Math.max(0, Math.min(resultsSnapshotIdx, len - 1))
    const iso = ts?.index[idx] ?? lts?.index[idx] ?? lints?.index[idx] ?? lkts?.index[idx] ?? ''

    const gMap = rowMap(ts, idx)
    const lMap = rowMap(lts, idx)
    const linMap = rowMap(lints, idx)
    ...
    const sdMap = rowMap(storageDispatchTS as TSPayload | null, idx)
    const stMap = rowMap(storeDispatchTS as TSPayload | null, idx)
    ...
    const socMap = rowMap(storageSoCTS as TSPayload | null, idx)
    const eMap   = rowMap(storeEnergyTS as TSPayload | null, idx)
    ...
    const qMap = rowMap(linesReactiveTS as TSPayload | null, idx)
    ...
    const linkMap = rowMap(linksTS as TSPayload | null, idx)
    ...
    const periodFromIso: number | null = (
      tsAny?.periods && idx < tsAny.periods.length ? tsAny.periods[idx] : null
    )
