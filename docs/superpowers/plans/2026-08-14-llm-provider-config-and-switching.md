...
- **ADR-0002 binds this entire plan** (`pypsa-gui/docs/adr/0002-chat-changes-need-a-live-api-probe.md`): no test in `backend/tests/` constructs a real client, so a green suite does NOT verify a chat change — "its absence is a defect in the change, not in the suite". Every task touching the chat path names the live probe it ran in its report, or states explicitly that its diff cannot reach a provider (pure config/store tasks legitimately can't). Task 11 upgrades the live Ollama smoke from optional to **required before the plan is called done**, and the Anthropic path gets one live turn on a real key. *Known debt: the merged seam (plan 1) shipped against Fake/MockTransport only, with no live probe — an ADR-0002 gap on already-merged work, closed by Task 11's probes covering both wires.*
- **ADR-0001 binds every empty/failed state** (`0001-unresolvable-figures-ship-as-null.md`): "a defaulted zero is indistinguishable from a real zero… a wrong number presented with the same confidence as a right one." Applies beyond numbers — a profiles fetch that was REFUSED must never render as "no models configured", and the dropdown/settings pane must distinguish unavailable from empty (Tasks 12–15).
...
- **MERGE PRECONDITION — this branch arms a tripwire.** On `master` today, `redact_secrets_in_str` is pattern-only (`sk-ant-*`, `key=value`, `bearer …`) with no literal-managed-value substitution, and that is harmless *only* because `llm_openai_compat` has no production caller there — no third-party key ever flows through a log or persist path. **This branch removes both halves of that safety**: Task 6/7 give the provider a real caller, and Tasks 1–3 introduce per-profile key slots for OpenAI/Moonshot/DashScope/custom endpoints whose values match none of the master patterns. The compensating control is Task 4's value-substitution widening (`live_secret_values()`, ≥8-char floor, substitute-before-regex, longest-first) — it is on this branch and NOT on master. **Therefore: Tasks 1–3 and 6–7 may never reach master without Task 4.** A partial merge, cherry-pick, or revert that lands provider wiring without the redaction widening ships a live third-party key into logs and `chat.jsonl` unscrubbed. Task 16 must assert this explicitly before the branch is declared done. Related: `docs/superpowers/findings/2026-08-27-lock-holder-email-reaches-the-model.md` (a separate, unowned Medium on master).
- **Error frames must not carry identifiers.** The spec already caps URLs at host:port; extend the same rule to identities. New kinds (`capability_unsupported`, `profile_switch_requires_new_chat`, `unreachable`) and any connection-test verdict must never emit an email, user id, org id, or project uuid into a `tool_result`, an SSE frame, or `chat.jsonl` — redaction will not catch these, because it is secrets-only by design.
...
- M6 in `llm_openai_compat`: send BOTH `max_tokens` and `max_completion_tokens` unless Task 2's research recorded a reason not to; synthesize `tool_use_start` when the FIRST delta for an index lacks `id`, so a tool block never ships `id: ""`. **CORRECTED 2026-08-27 — do NOT use `f"call_{index}"`**, which was this plan's original text and is wrong: upstream mints ids of exactly that shape, so a real `call_0` arriving on index 1 collides with a synthesized `call_0` on index 0, two tool_use blocks share an id, and tool_use_id↔tool_result pairing breaks. Use a prefix upstream will not mint (`f"__synth_{index}"`) and disambiguate against every real id seen in the stream before finalising. Emit `tool_use_start` **at most once per index**: if a real id arrives on a later delta for an index already started, adopt it for the final block (tool_result must carry upstream's id) but emit no second start — the naive version fires twice and orphans the first `tool_preparing` frame, a regression on a path that previously worked. T6: a user turn mixing plain text + tool_result emits BOTH the tool messages and a user message (drop the `and not results` guard).
...
- [ ] **Assert the merge precondition (see Global Constraints).** Run `git log --oneline <plan-start>..HEAD -- pypsa-gui/backend/services/redaction.py` and confirm Task 4's value-substitution widening is present in the same set as the provider wiring. Then prove it end-to-end rather than by inspection: set a non-Anthropic managed key to a value matching none of the master patterns (no `sk-ant-`, no `key=`, no `bearer`), drive one turn that logs and persists an error carrying it, and grep the log file and `chat.jsonl` for the literal value — it must be absent. Paste the command and its output. **If this branch is ever split, cherry-picked, or partially reverted, this assertion is what catches a provider landing without its redactor.**
