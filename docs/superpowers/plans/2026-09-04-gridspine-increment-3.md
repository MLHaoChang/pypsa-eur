...
- **LANDED 2026-09-05.** The LODF crosses the cage as a schema-level artifact, `schema.dc.DCSensitivities` (PTDF, LODF, islanding mask, ratings, bus and branch names; validated; saved as `dc_sensitivities.npz` by the driver), produced by `static.lodf.to_sensitivities` and consumed by `ranking.severity` — which rebuilds each hour's bus injections from the dispatch and loads tables and multiplies; proven equal to the static-side DC solve for one hour to 1e-6 MW. Islanding outages are EXCLUDED from the severity for the reason task 4 found. `_RANKING` gained `max_n1_severity`; selection is now k..5k. **Blind-spot measurement:** over 15 synthetic hours (load 0.6–1.0 × RES cf 0.1–0.6) DC and AC severity rank identically — Spearman rho 1.00, worst rank gap 0 — but the sample is monotone scalings dominated by the same three base-case overloads and does not exercise the reordering task 5's underestimation could cause; task 13 should re-measure on the UC-dispatched year's selected hours. That figure is the `dc_severity_blind_spot` the ledger README declares.
...
- **LANDED 2026-09-05.** Ranking is unchanged; only the k-th BOUNDARY tie is spread: when the hours tied at the boundary value outnumber the slots left for them, those slots are filled by a farthest-point pass over hour-of-year seeded at the earliest tied hour (equal distances to the earlier hour). Strictly better hours are never displaced, a tie that fits its slots is untouched, and a single remaining slot still goes to the earliest tied hour — so every increment-2 expectation holds, including `test_ties_are_broken_by_the_earliest_hour`. Linear hour-of-year distance, not circular (31 Dec and 1 Jan are far apart), a deliberate simplicity. Hand check pinned: twenty tied hours 10..29 with five slots select 10, 14, 19, 24, 29. The 2026-09-04 selection (five January hours at the floor) is now superseded and the full-year run must be redone before any PowerFactory validation — as agreed.
