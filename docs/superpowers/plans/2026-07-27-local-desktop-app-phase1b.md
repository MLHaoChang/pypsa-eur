...
`pypsa-gui/backend/projects/` holds **113 MB**: 13 directories (12 with a depth-1 `network.nc`, one of them the org-scoped tree) plus one `.zip`. Nothing was deleted, and the one DB-tracked project still resolves because its `storage_path` is absolute:
...
| 9 | Legacy tree: **13 directories** (12 with depth-1 `network.nc`, incl. `new_project_test.pypsaproj` — a real 22 MB project — and the org tree) + one `.zip` | Classify by content, never by suffix. |
...
**Files:** Modify `backend/services/storage_paths.py`, `backend/services/project_registry.py:171,207,225-238`, `backend/services/legacy_migrate.py:243`, `backend/db/models.py:42`, the **eight direct test call sites** (`tests/test_project_locks.py:105`; `tests/test_projects_tenancy.py:107,245,383,505,522`; `tests/test_project_acl.py:84,122`), **and the two modules that assert the layout those callers produce** — `tests/test_unclaimed_import.py:217-224` (builds `projects_root/<org>/<project_id>` and asserts `project.storage_path == str(destination)`) and `tests/test_legacy_migrate.py:155`; create `backend/alembic/versions/0003_relative_storage_path.py`, `backend/tests/test_storage_layout.py`
