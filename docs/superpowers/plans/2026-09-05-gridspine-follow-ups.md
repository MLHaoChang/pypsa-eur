...
Steps:
1. `_screen_units` on the shared GridModel: `deactivate_gen`/`deactivate_sgen` + `ac_pf` from the base voltages, `get_lineor_res`/`get_trafohv_res` currents through `branch_loading_pct`; `reactivate` after each. pandapower stays the oracle in the tests (existing `test_a_unit_outage_matches_a_pandapower_solve_with_the_gen_out`, plus the decommitted-hour ones). Pin: `G_BUS_39` outage still a recorded collapse (ruling 16) — an empty `ac_pf` result is `converged=False, islanded=False`. Measure and record the per-hour time.
2. `n1_severity_ac(net, contingencies, dispatch, loads, registry, hours) -> pd.Series` in `static/contingency.py`: per hour `apply_snapshot` on a work copy, `screen_n1`, worst severity over converged non-islanded rows; `NaN` for a non-converging base case (do not raise across a year). One code path with the screen by construction; test pins equality with a direct `screen_n1` at three hours.
3. Driver: `metrics["n1_severity_ac"]` from step 2 over all hours, `n1_severity_dc` kept; `_RANKING`'s `max_n1_severity` reads `n1_severity_ac`. The blind-spot measurement moves to ALL hours (rho of DC vs AC over the year, worst rank gap) and the selected-hour one stays in the ledger as before. Manifest gains `ac_year_pass_seconds`.
4. Runtime: the year-study module's fixture measured **326 s** on 2026-09-05 (336 h, k=2, screening on; 4 cores, partly under contention with probes) — the increment-2 "<120 s" budget was already gone once Task 13 put N-2 and fault levels on up to 10 selected hours, and nobody re-measured. F2 adds ~336 × 70 ms ≈ 25 s for the year pass. Measure before and after; record both in the test module docstring; if the module is still over five minutes, cut the fixture to 168 h/k=2 (one window) rather than skipping screening. Replace the "≤4 AC snapshots" rule in the increment-3 plan with the measured budget.
5. Mutation: make the criterion read `n1_severity_dc` again → the driver test that pins the selected `max_n1_severity` hour equal to the argmax of the AC column goes red.
