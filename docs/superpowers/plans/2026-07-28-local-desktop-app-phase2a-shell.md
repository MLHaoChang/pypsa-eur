...
- **The desktop environment matches `default` on the core stack and the backend's import closure, NOT fully.** ~90 further packages (build tooling, docs and plotting, snakemake-only paths) sit on newer patch versions. Closing it needs a pixi solve-group, which re-solves `default` and upgrades the model environment — measured at 228 lines including pypsa 1.2.4 and highspy 1.15.1, and rejected on those grounds. `tests/test_desktop_environment.py` enforces the part that is enforced.
