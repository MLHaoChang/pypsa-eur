# Phase 12h — a static capacity factor is applied, and "it already includes outages" is a flag the asset carries (plan v5 — scope cut to the finding it adjudicates)
...
**v1–v4 were rejected** (four adversarial reviews, 57 findings, five
blockers; the full records are the appendices of
...
- `_bulk`'s bool branch clears `null` to the column's class default —
  `p_nom_extendable False`, `committable False`, `maintainable False`, and
  `False` for a custom column — closing a **reachable 500** (the bulk editor
  sends `null` for a blank cell). **`active` is the exception and is refused
  422** naming `true`/`false`: its default is `True`, so clearing it would
  *activate* every selected asset, and the confirm toast says "Set active =
  (unset) on 200 generator(s)?" — a sentence that must not mean "switch 200
  assets on".
...
**no informative series exists**, a static `p_max_pu ≠ 1` (finite) **scales
the unit's capacity** — `capacity_mw × cf`, and `capacity_series × cf` where
12d gave the unit a per-period series — with `profile` left `None` and
`folded_constant = cf` recorded as payload data. Applied only in the absence
of a series (beside one it would be read twice), and **on the occurrence-unit
branch only**: `fleet_and_residual`'s must-take branch already applies the
...
`profile_and_outage_modelled` keeps its population (asset-typed, varying
series, `q > 0`) and its sentence, which is true of it; it loses its false
...
`folded_units` — `(name, folded_constant, source: "static")` — and
`deterministic_units`. Both are needed because a folded unit is in **no**
existing list (`profile_units` is `mixed + netted`, and the fold empties its
membership), and a deterministic unit likewise leaves `profile_units` after
H2b; `fidelity_note`'s counts stay true for what remains.
...
**up**, because the unit was being credited at nameplate. A PyPSA-Eur import
...
- ★ H1b — the data model, every path the reviews broke: a first `POST` on a
  ...
  **a solve with `voll > 0` followed by a project save returns 200 and the
  ...
  and `p_nom_extendable: null` read `False` where today each is a 500; and
  **`active: null` is 422**, naming `true`/`false`.
...
- ★ H3a: static `0.8` ≡ scaled capacity ≡ a constant series `0.8`: COPT
...
- ★ H3c: 12d interaction — a static CF on a unit with a per-period capacity
  ...
  only (the 12d branch takes `max(capacity_series)`, so it bites).
- ★ H3d: the must-take branch is untouched — a must-take farm with a static
  ...
  the `source != "missing"` branch — the CF is squared.
...
- ★ H4d: `/copt` and `/mc` carry `folded_units` and `deterministic_units`, on
  **single-block and multi-block** fixtures; `reserve_margin_facts["carrier_default"]`
  ...
  Bites: drop each list; drop the `carrier_default` skip.
...
- Docs: `copt.py`'s module docstring and `_occurrence_profile`'s "the static
  column is deliberately NOT read"; the design spec's and the MC spec's
...
Each full record lives in its own file; nothing was deleted.
